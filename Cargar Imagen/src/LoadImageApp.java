...
public class LoadImageApp extends JFrame {
    ...
    private void drawHistogram(Graphics g, int[] redHistogram, int[] greenHistogram, int[] blueHistogram) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            g.fillRect(startX + i * (barWidth + spacing), startY + histogramHeight + 10 - barHeight, barWidth, barHeight);
        }
    
        g.setColor(Color.BLUE);
        for (int i = 0; i < 256; i++) {
            int barHeight = (int) ((double) blueHistogram[i] / maxFrequency * histogramHeight);
            g.fillRect(startX + i * (barWidth + spacing), startY + 2 * (histogramHeight + 10) - barHeight, barWidth, barHeight);
        }
    }
